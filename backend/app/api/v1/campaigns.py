...
def get_campaigns(organization_id: int, db: Session = Depends(get_db)):
...
def create_campaign(organization_id: int, campaign_in: CampaignCreate, db: Session = Depends(get_db)):
...
def get_campaign(organization_id: int, campaign_id: int, db: Session = Depends(get_db)):
...
def update_campaign(organization_id: int, campaign_id: int, campaign_in: CampaignUpdate, db: Session = Depends(get_db)):
...
async def generate_content(organization_id: int, campaign_id: int, request: GenerateContentRequest, db: Session = Depends(get_db)):
