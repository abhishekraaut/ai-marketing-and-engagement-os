...
def submit_review(organization_id: int, content_id: int, variant_id: int, db: Session = Depends(get_db)):
...
def approve(organization_id: int, content_id: int, variant_id: int, db: Session = Depends(get_db)):
...
def reject(organization_id: int, content_id: int, variant_id: int, request: RejectRequest, db: Session = Depends(get_db)):
...
def edit_variant(organization_id: int, content_id: int, variant_id: int, data: VariantUpdate, db: Session = Depends(get_db)):
