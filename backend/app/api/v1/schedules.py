...
def schedule_variant(organization_id: int, variant_id: int, data: ScheduleCreate, db: Session = Depends(get_db)):
...
def get_calendar(organization_id: int, db: Session = Depends(get_db)):
