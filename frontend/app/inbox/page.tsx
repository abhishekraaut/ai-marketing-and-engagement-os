    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 shrink-0">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">Manage, filter, and reply to community interactions with AI assistance.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-slate-50 transition-colors disabled:opacity-50 shadow-sm"
          >
      ...
      <div className="flex flex-col md:flex-row flex-1 gap-6 min-h-0">
        ...
        {/* Left Pane - List */}
        <div className="w-full md:w-5/12 lg:w-1/3 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden shrink-0">
          
          <div className="p-3 border-b border-slate-100 bg-slate-50/50 flex gap-2 overflow-x-auto scrollbar-hide shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 relative">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center px-4 py-12">
                ...
                <h3 className="font-semibold text-slate-900 text-sm">Inbox Zero</h3>
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="font-bold text-slate-900 text-sm truncate">{item.author_name}</span>
                    </div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider shrink-0 ml-2">{item.platform}</span>
                  </div>
                  <p className={cn(
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    {item.reply_status === 'NEEDS_REPLY' && <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full font-bold">Needs Reply</span>}
                  </div>
                </div>
              ))
            )}
        ...
        {/* Right Pane - Detail */}
        <div className="w-full md:w-7/12 lg:w-2/3 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col min-h-[500px] overflow-hidden relative">
          {!selectedItem ? (
            ...
            <>
              <div className="p-6 border-b border-slate-100 bg-white shrink-0">
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-slate-900 tracking-tight">{selectedItem.author_name}</h2>
                ...
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 relative">
                  ...
                  <p className="text-slate-800 text-[15px] leading-relaxed pt-2">{selectedItem.content}</p>
                  <div className="flex items-center gap-2 text-xs text-slate-400 mt-4 pt-4 border-t border-slate-200/60 font-medium">
              ...
              <div className="p-6 flex-1 flex flex-col bg-slate-50/30 overflow-y-auto">
                <div className="flex items-center gap-2 mb-4">
                  <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider">Reply Workflow</h3>
                        ...
                        <div className="flex-1 flex flex-col animate-in fade-in zoom-in-95 duration-200">
                          ...
                          <div className="flex justify-end gap-3 mt-4">
                            <button 
                              ...
                              className="px-4 py-2 text-slate-500 font-semibold hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors flex items-center gap-1.5"
                            >
                            ...
                            <button 
                              ...
                              className="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50 flex items-center gap-1.5"
                            >
                    ...
                    <div className="flex flex-wrap gap-3 justify-end shrink-0">
                      {['AI_DRAFTED', 'APPROVED'].includes(selectedItem.reply_status) && !editMode && (
                        <button 
                          ...
                          className="px-6 py-2.5 text-slate-700 bg-white border border-slate-200 rounded-xl font-bold hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm flex items-center gap-2"
                        >
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] animate-pulse">
      ...
      <div className="flex flex-1 gap-6">
        <div className="w-1/3 bg-white rounded-xl border border-slate-200 p-4 space-y-4">
          <div className="h-10 bg-slate-100 rounded-lg" />
          {[1,2,3,4].map(i => <div key={i} className="h-24 bg-slate-50 rounded-xl" />)}
        </div>
        <div className="w-2/3 bg-white rounded-xl border border-slate-200" />
      </div>
    </div>
  );
}
