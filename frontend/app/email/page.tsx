    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="page-description">Manage and automate your email marketing workflow with AI generation.</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-xl shadow-sm border border-border overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-16 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-bold text-foreground mb-2">No email campaigns yet</h3>
            <p className="text-muted-foreground max-w-sm mb-6">
              Create your first email campaign to start engaging with your audience.
            </p>
            <button 
              onClick={() => setShowModal(true)}
              className="flex items-center gap-2 text-primary font-bold hover:text-primary-hover"
            >
...
<table className="app-table">
              <thead className="bg-muted">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Audience</th>
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {emails.map((email: EmailCampaign) => (
                  <tr key={email.id} className="hover:bg-muted/50 transition-colors group">
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <Link href={`/email/${email.id}`} className="text-sm font-bold text-foreground hover:text-indigo-600 transition-colors">
                            {email.name}
                          </Link>
                          <div className="text-xs text-muted-foreground mt-0.5">ID: {email.id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="flex items-center gap-2 text-sm text-slate-600">
                        ...
                        <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">{email.recipient_count || 0}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-sm">
                      <div className="max-w-xs truncate text-slate-700 font-medium">
                        {email.subject || <span className="text-muted-foreground italic">No subject set</span>}
                      </div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between p-6 border-b border-border bg-muted/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-foreground leading-tight">New Email Campaign</h2>
                  <p className="text-xs text-muted-foreground mt-0.5">Configure your audience and links.</p>
                </div>
              </div>
              <button 
                onClick={() => setShowModal(false)}
                className="text-muted-foreground hover:text-slate-600 transition-colors p-2 hover:bg-muted rounded-lg"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <input 
                  type="text" 
                  className="w-full border-2 border-border rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 font-medium transition-all"
                  value={formData.name}
              ...
              <div>
                ...
                <select 
                  className="w-full border-2 border-border rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 font-medium bg-white transition-all appearance-none cursor-pointer"
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1.5">
                  Parent Marketing Campaign <span className="text-muted-foreground font-normal">(Optional)</span>
                </label>
                <select 
                  className="w-full border-2 border-border rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 font-medium bg-white transition-all appearance-none cursor-pointer"
            ...
            <div className="p-6 border-t border-border bg-muted/50 flex justify-end gap-3">
              <button 
                onClick={() => setShowModal(false)}
                className="px-5 py-2.5 text-sm font-bold text-slate-600 hover:text-foreground bg-white border border-border hover:border-slate-300 rounded-xl shadow-sm transition-all"
              >
                Cancel
              </button>
              <button 
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className="inline-flex items-center gap-1.5 bg-muted text-slate-700 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">{status}</span>;
}
